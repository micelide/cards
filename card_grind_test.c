...
void getName(struct player *a, int x, char *name)
...
{//play the game with 2 players
	...
	{ //if the players have at least 21, check for a 2 point lead
		...
		{//start the game over with a new deck
			printf("End of deck reached without a winner, reshuffling to continue...\n");
			play2(p1, p2,deck);
		}
	}
	else
...
int main(int argc, char **argv)
{
	...
	sleep(1); //makeup for user input in getNumPlayers()
	int numplayers = atoi(argv[1]);//getNumPlayers(temp);
	if(numplayers == 0)
